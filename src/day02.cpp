...
std::tuple<long long, long long> parseBounds(std::string boundsString)
{
    // e.g. 00000-99999
...
bool doNumbersRepeatPart2(long long number)
{
    ...
    // We are looking for repeats so we can skip the whole number factor which is set to the last number in the vector
    factors.pop_back();
...
std::vector<long long> repeatingValues(long long lowerBounds, long long upperBounds, bool part2 = false) // This wouldn't exist in reality
{
    ...
    for (size_t i = lowerBounds; i <= upperBounds; i++)
    {
