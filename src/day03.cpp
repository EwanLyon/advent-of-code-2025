...
int largestPossibleNumberPart1(std::vector<int> numbers)
{
    int largestNumber = -1;
...
long long largestPossibleNumberPart2(std::vector<int> numbers, int size)
{
    ...
    long long result = 0;
    for (size_t i = 0; i < largestNumber.size(); i++)
    {
        int exponent = largestNumber.size() - 1 - i;
        long long powerOf10 = std::pow(10, exponent);

        result += largestNumber[i] * powerOf10;
    }
